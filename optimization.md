...
| 阶段 | 内容 | 预期 QPS（8读2写） | 改动量 |
|------|------|-------------------|--------|
| **Phase 1** | 分段锁（16 shard） | 1.0M → 4~6M | ~100行 |
| **Phase 2** | 柔性数组 + Arena | 4~6M → 5~8M | ~200行 |
| **Phase 3** | Lock-Free CAS（终极方案） | 5~8M → 10M+ | 完全重写 |

Phase 1 和 Phase 2 可以并行推进（改动独立）。Phase 3 是重构级变更，建议在 Phase 1+2 验证通过、需求明确后再启动。

---

## 与工业级对比（参考基准）

| 指标 | 本实现 | RocksDB Skiplist | Java ConcurrentSkipListMap |
|------|--------|------------------|---------------------------|
| 并发模型 | 全局 shared_mutex | Lock-Free CAS | Lock-Free CAS |
| 纯写 QPS | ~340k | 3~10M（估算） | 2~5M（估算） |
| 读写混合 QPS | ~1M | 5~15M（估算） | 5~10M（估算） |
| 内存模型 | 离散 malloc | Arena + FAM | JVM 托管 |
| 持久化 | 手动全量 dump | 不负责持久化 | 不负责持久化 |

**说明**：RocksDB/JDK 的绝对 QPS 高度依赖硬件和 workload，以上为数量级估计，仅供参考。核心差异在于锁模型——Lock-Free 方案在高并发下可线性扩展，全局锁在 10 线程附近就触顶。
